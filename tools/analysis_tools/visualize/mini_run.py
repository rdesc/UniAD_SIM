...
class Visualizer:
    ...
    def _parse_predictions(self, outputs):
        if self.show_sdc_traj:
            outputs['boxes_3d'].tensor = torch.cat(
                [outputs['boxes_3d'].tensor, outputs['sdc_boxes_3d'].tensor], dim=0)
            outputs['scores_3d'] = torch.cat(
                [outputs['scores_3d'], outputs['sdc_scores_3d']], dim=0)
            outputs['labels_3d'] = torch.cat([outputs['labels_3d'], torch.zeros(
                (1,), device=outputs['labels_3d'].device)], dim=0)
        # detection
        ...
        if self.with_map:
            map_thres = 0.7
            score_list = outputs['pts_bbox']['score_list'].transpose([
                1, 2, 0])
            predicted_map_seg = outputs['pts_bbox']['lane_score'].transpose([
                1, 2, 0])  # H, W, C
        ...
        if self.with_planning:
            # detection
            bboxes = outputs['sdc_boxes_3d']
            scores = outputs['sdc_scores_3d']
            labels = 0
